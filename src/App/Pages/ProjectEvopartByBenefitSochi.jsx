...
export default function ProjectEvopartByBenefitSochi() {
  ...
  useEffect(() => {
    ...
    let startY = 0; // Добавляем для отслеживания Y-координаты
  ...
  const ArrayOfPictures = Array.from({ length: 7 }, (_, index) => ({
    src: `EVOPART BY BENEFIT SOCHI-${index + 1}`,
  }));
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="parallax-container relative overflow-hidden md:h-[50rem] h-[30rem] flex items-center justify-center mt-24">
        <div
          className="parallax-bg absolute inset-0 bg-[url(/img/EVOPARTBYBENEFITSOCHI.jpg)] md:bg-[url(/img/EVOPARTBYBENEFITSOCHI.jpg)] bg-cover bg-no-repeat h-[120%] w-full transform-gpu"
          style={{
      ...
      <div className="flex w-full py-10 flex-col gap-10 lg:flex-row lg:gap-0">
        <div className="flex-[3] self-center justify-items-center 2xl:pl-30 xl:pl-20 px-10">
          <h1 className="max-w-[600px] text-4xl text-center font-share-tech-mono">
            EVOPART BY BENEFIT SOCHI
          </h1>
          <br />
          <p className=" max-w-[600px] text-center font-share-tech-mono">
            Краснодарский край, г.Сочи, Центральный р-н, улица Курортный
            проспект 46/8
          </p>
        </div>
        <div className="flex-[6] self-center justify-items-center px-10">
          <p className="max-w-[600px] font-share-tech-mono">
            Эксклюзивный проект c атмосферой элитного закрытого клуба,
            расположенный в центре Сочи. Особое расположение в сердце города,
            исключительная приватность, безопасность, современная архитектура и
            современный дизайн. Архитекторы вдохновлялись совершенными изгибами
            Чёрного моря и силуэтами гор Кавказа. Пять непрерывных пластичных
            волн создают эффект парящей над городом уникальной архитектуры
            комплекса и органично вписывают проект в городской ландшафт, являясь
            продолжением плавного движения улицы Морской. Номерной фонд EVOPART
            BY BENEFIT SOCHI CENTER представлен просторными пространствами
            премиум-класса площадью от 50 кв.м до 83 кв.м. Панорамное остекление
            и безупречная шумоизоляция позволит в полной мере насладиться
      ...
      <div className="relative w-full px-12 mx-auto my-8">
        ...
        <div className="overflow-hidden">
          <div className="carousel flex transition-transform duration-500 ease-in-out gap-6">
            {/* Изображения */}
